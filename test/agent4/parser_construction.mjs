...
import {bufferUntilEOF} from '../consumers/incremental-parser/candidates.mjs';
...
const input=[subject,model,trace,17n];
...
for(let round=0;;round++) {
  ...
  if(out.kind==='requested') {
    ...
    if(request.semanticIdentity==='agent.parser.reference.v1') {
      ...
      assert.deepEqual(events,['agent.parser.reference.v1','agent.model.invoke.v3']);
      assert.match(payload[3].at(-1)[1],/observations: 3$/);
      assert.deepEqual(payload[4].map(tool=>tool[2]),['fragment','unresolved']);
      const explanation='Escape handling is present; record emission still needs consumer assessment.';
      reply={tag:0,value:[[{tag:0,value:['same-provider-id','fragment',
        new TextEncoder().encode(JSON.stringify({source:bufferUntilEOF,explanation})),0,
        {tag:0,value:{tag:0,value:[bufferUntilEOF,explanation]}}]}],Array(32).fill(0)]};
    } else if(request.semanticIdentity==='agent.parser.execution.v1') {
      assert.equal(payload[2][0],bufferUntilEOF);assert.equal(payload[2][1],1n);assert.equal(payload[2][2],0);
      assert.equal(payload[1],18n);reply=await tools.execute(payload);
...
assert.deepEqual(events,['agent.parser.reference.v1','agent.model.invoke.v3','agent.parser.execution.v1']);
assert.equal(result.tag,2,'consumer must return its actual assessment');
assert.equal(result.value[0][0],bufferUntilEOF);assert.equal(result.value[0][2],0);
assert.equal(result.value[1][2].tag,0);assert.equal(result.value[1][2].value[1],false,'actual probe must refute EOF-only emission');
assert.deepEqual(result.value[1][2].value[0],[
  [[],0,{tag:0,value:null}],[[],0,{tag:0,value:null}],[[[[10]]],1,{tag:0,value:null}],
],'the counterexample is delayed record emission, not an unrelated execution failure');
console.log(JSON.stringify({imageBytes:image.length,events,transfers,engines,browser:browser?.identity,workersDestroyed:browser?.workersDestroyed,partialCandidate:true,
  counterexampleObserved:true,metrics:tools.metrics(),paidProviderCalls:0}));
