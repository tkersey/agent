...
async function request(outcome, identity) {
  assert.equal(outcome.kind, "requested");
  const req = (await world.decodeRequest(outcome.request));
...
async function reply(outcome, response) {
  ...
  const result = (await world.encodeResult(outcome.request, bytes));
  return invoke({image, state: outcome.state, control: "reply", value: result});
}
async function begin(proposal = base(), occurrence = 100n) {
  const initialArgs = encodeValue(proposalSchema, proposal);
  const issued = await invoke({image, initialArgs});
  assert.deepEqual((await request(issued, "agent.approval.issue.v1.probe.document")).decoded, proposal);
  const approval = await reply(issued, occurrence);
  const req = (await request(approval, "agent.interaction.exchange.v1.probe.document"));
...
test("approve consumes authority once and preserves all environmental delivery outcomes", async () => {
  for (const tag of [0, 1, 2, 3]) {
    ...
    assert.deepEqual((await request(commit, "agent.tool.document.replace.v1")).decoded, base());
...
test("amendment retires the old challenge and requires another trusted occurrence", async () => {
  ...
  assert.deepEqual((await request(issuance, "agent.approval.issue.v1.probe.document")).decoded, amended);
  const next = await reply(issuance, 101n);
  const current = (await request(next, "agent.interaction.exchange.v1.probe.document")).decoded[3];
  ...
  assert.deepEqual((await request(commit, "agent.tool.document.replace.v1")).decoded, amended);
});
  ...
  for (const matches of [false, true]) for (const principal of [false, true])
    for (const decision of [0, 1, 2]) for (const live of [false, true])
      for (const current of [false, true]) {
        ...
        if (expected === "commit") (await request(actual, "agent.tool.document.replace.v1"));
        else if (expected === "issue") (await request(actual, "agent.approval.issue.v1.probe.document"));
...
test("stale ERS3 and malformed typed input preserve the parked authoritative State", async () => {
  const first = await begin(base(), 100n);
  const other = await begin([1, 42n, "Other occurrence", true], 101n);
  const current = (await world.decodeRequest(first.approval.request));
  const canonical = encodeValue(decodeSchema(current.resumeSchema),
    value(0, [first.challenge, 7, value(0, null)]));
  const stale = (await world.encodeResult(first.approval.request, canonical));
  const saved = Buffer.from(other.approval.state);
  await assert.rejects(invoke({image, state: other.approval.state, control: "reply", value: stale}));
  assert.deepEqual(Buffer.from(other.approval.state), saved);
  await assert.rejects(world.encodeResult(other.approval.request, Uint8Array.of(255)));
  assert.deepEqual(Buffer.from(other.approval.state), saved);
  (await request(await decide(other.approval, other.challenge, value(0, null)),
...
test("required live proof binds the base across approval and every amendment", async () => {
  ...
  const respond = async (outcome, response) => {
    ...
    return invoke({image: evidenceImage, state: outcome.state,
      control: "reply", value: (await world.encodeResult(outcome.request, canonical))});
  };
  async function live(version) {
    const read = await invoke({image: evidenceImage,
      initialArgs: encodeValue(proposalSchema, base())});
    (await request(read, "agent.tool.document.read.version.v1"));
    return respond(read, version);
  }
  completed(await live(43n), value(3, null));
  const issued = await live(42n);
  (await request(issued, "agent.approval.issue.v1.probe.document"));
  const pending = await respond(issued, 200n);
  const challenge = (await request(pending, "agent.interaction.exchange.v1.probe.document")).decoded[3];
  ...
  assert.deepEqual((await request(newIssued, "agent.approval.issue.v1.probe.document")).decoded, retainedBase);
  const next = await respond(newIssued, 201n);
  const nextChallenge = (await request(next, "agent.interaction.exchange.v1.probe.document")).decoded[3];
  const commit = await respond(next, value(0, [nextChallenge, 7, value(0, null)]));
  assert.deepEqual((await request(commit, "agent.tool.document.replace.v1")).decoded, retainedBase);
});
...
test("scoped current-policy cells survive fresh-instance approval resumption", async () => {
  for (const evidence of [false, true]) {
    ...
    const respond = async (outcome, response) => {
      const req = (await world.decodeRequest(outcome.request));
      const canonical = encodeValue(decodeSchema(req.resumeSchema), response);
      return invoke({image: scopedImage, state: outcome.state,
        control: "reply", value: (await world.encodeResult(outcome.request, canonical))});
    };
    for (const allowed of [false, true]) {
      let issued = await invoke({image: scopedImage, initialArgs: Uint8Array.of(Number(allowed))});
      if (evidence) {
        (await request(issued, "agent.tool.document.read.version.v1"));
      ...
      assert.equal((await request(issued, "agent.approval.issue.v1.probe.scoped")).decoded, 42n);
      const pending = await respond(issued, 100n);
      const challenge = (await request(pending, "agent.interaction.exchange.v1.probe.scoped")).decoded[3];
      const outcome = await respond(pending, value(0, [challenge, 0n, value(0, null)]));
      if (allowed) assert.equal((await request(outcome, "agent.tool.scoped.commit.v1")).decoded, 42n);
      else {
        assert.equal(outcome.kind, "completed");
