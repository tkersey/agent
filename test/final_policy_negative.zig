...
const Definition = agent.define(.{
    .name = "final-policy-negative",
    .version = "1.0.0",
    .instructions = "Never finish before tests pass.",
    .Goal = u32,
    .Action = Action,
...
const Machine = agent.compile(Definition, agent.strategy.react(.{}), agent.epistemics.verbatim(.{
    ...
    .overflow = .fail,
    .final = agent.final_policy.latestObservationBool(.run_tests, .passed, true),
}), .{
    .machine = .{
        .maximum_frames = 32,
...
fn expectInvalidVariant(state: Machine.State, fuel: *u64) !void {
    switch (try Machine.step(state, fuel)) {
        ...
        else => return error.PrematureFinalWasAccepted,
    }
}

test "final policy rejects final with no observation" {
    ...
    try resumeRequest(&state, decision, Action{ .final = 1 });
    try expectInvalidVariant(state, &fuel);
}

test "final policy rejects final after a failing test observation" {
    var state = try Machine.initialState(std.testing.allocator, @as(u32, 1));
    defer Machine.deinitState(state);
    var fuel: u64 = 4096;
    const decision = switch (try Machine.step(state, &fuel)) {
    ...
    try resumeRequest(&state, final_decision, Action{ .final = 1 });
    try expectInvalidVariant(state, &fuel);
}
