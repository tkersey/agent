...
const invoke = async input => { const kernel = await world.Kernel.create({ bytes: kernelBytes, expectedSha256: identity.kernelSha256 }); kernel.setLimits({ input: 256 << 20, working: 256 << 20, output: 256 << 20 }); const bytes = kernel.invoke(world.encodeInput(input)); return { ...world.decodeOutcome(bytes), bytes }; };

async function start(mode) {
  const image = await readFile(path.join(images, `${mode}.bpi3`));
  const initialArgs = await readFile(path.join(images, `${mode}.args`));
  return { image, outcome: await invoke({ image, initialArgs }) };
}

async function pending(session, expected) {
  assert.equal(session.outcome.kind, "requested");
  const request = (await world.decodeRequest(session.outcome.request));
...
async function canonicalAnswer(session, encoded) {
  session.outcome = await invoke({ image: session.image,
    state: session.outcome.state, control: "reply", value: (await world.encodeResult(session.outcome.request, encoded)) });
}
...
async function modelReply(session, question, answerValue) {
  const request = (await pending(session, "agent.model.invoke.v3"));
  const invocation = decodeModelInvocation(request.payload);
...
async function transfer(session) {
  ...
  // The new instance receives only image + detached PST3. Pending operation is reconstructed.
  session.outcome = await invoke({ image: Uint8Array.from(session.image),
...
test("same non-authority Ask body agrees under human, model, and rule responders", async () => {
  ...
  assert.equal(payload((await pending(human, "agent.interaction.exchange.v1.review.answer")))[3], 7);
...
async function reviewTurn(session, mode, input, evidence, rawScore, clarified, previous) {
  const traces = [];
  const read = (await pending(session, "review.evidence.read.v1"));
  ...
  const clarification = async (outgoing) => {
    const request = (await pending(session, "agent.interaction.exchange.v1.review.clarification"));
  ...
  const next = (await pending(session, "agent.interaction.exchange.v1.review.next"));
...
test("malformed interaction replies preserve the parked continuation", async () => {
  ...
  await assert.rejects(world.encodeResult(session.outcome.request, Uint8Array.of(2)));
...
test("every normalized model failure exits before the post-answer computation", async () => {
  ...
  for (const failure of failures) {
    const session = await start("model");
    (await pending(session, "agent.model.invoke.v3"));
