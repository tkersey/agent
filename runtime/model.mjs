...
import { decodeNatural, encodeNatural } from "./values.mjs";
...
function validateToolMetadata(tools) {
  for (const key of ["name", "actionOrdinal", "actionTag"]) {
    assert.equal(new Set(tools.map((tool) => tool[key])).size, tools.length,
      `duplicate model declaration ${key}`);
  }
  for (const tool of tools) {
    assert(tool.name.length > 0, "model declaration name is empty");
    const names = tool.argumentCodec.map((field) => field.name);
    assert.equal(new Set(names).size, names.length, "duplicate codec field name");
    for (const field of tool.argumentCodec) {
      assert(field.name.length > 0, "codec field name is empty");
      const integer = field.kind === "signed_integer" || field.kind === "unsigned_integer";
      assert(integer ? [8, 16, 32, 64].includes(field.bitWidth) : field.bitWidth === 0,
        "codec integer width is unsupported");
      assert(field.kind === "text" || field.maximumBytes === 0,
        "non-text codec has a text capacity");
      if (field.kind === "enumeration") {
        assert.equal(field.enumNames.length, field.enumTags.length,
          "enum codec names and tags differ in length");
        assert(field.enumNames.length > 0, "enum codec is empty");
        assert.equal(new Set(field.enumNames).size, field.enumNames.length,
          "duplicate enum codec name");
        assert.equal(new Set(field.enumTags).size, field.enumTags.length,
          "duplicate enum codec tag");
      } else {
        assert.equal(field.enumNames.length, 0, "non-enum codec declares enum names");
        assert.equal(field.enumTags.length, 0, "non-enum codec declares enum tags");
      }
    }
    const schema = parseJsonStrict(fatalUtf8(tool.inputSchemaJson));
    assert(schema !== null && schema.type === "object" &&
      schema.properties !== null && typeof schema.properties === "object" &&
      !Array.isArray(schema.properties), "model declaration requires object properties");
    assert.deepEqual(Object.keys(schema.properties).sort(), [...names].sort(),
      "model schema and codec fields differ");
    assert(Array.isArray(schema.required) || (schema.required === undefined && names.length === 0),
      "model schema required fields must be an array");
    assert.deepEqual([...(schema.required ?? [])].sort(), [...names].sort(),
      "model schema and codec required fields differ");
    assert.equal(schema.additionalProperties, false,
      "model schema admits unknown fields");
    for (const field of tool.argumentCodec) {
      const property = schema.properties[field.name];
      assert(property !== null && typeof property === "object", "model schema field is invalid");
      const type = field.kind === "text" || field.kind === "enumeration" ? "string"
        : field.kind === "boolean" ? "boolean" : "integer";
      assert.equal(property.type, type, "model schema and codec field types differ");
      if (field.kind === "enumeration") {
        assert.deepEqual(property.enum, field.enumNames, "model schema and codec enum names differ");
      }
    }
  }
}
...
function decodeActionArguments(text, toolName, tools, limits) {
  ...
  try {
    ...
    return Object.freeze({
      ...
      actionBytes: Buffer.concat([varuint(tool.actionOrdinal), encodedPayload]),
    });
  } catch (error) {
    return Object.freeze({
      kind: "invalid",
...
function encodeOutputItems(items) {
  const encoded = [varuint(items.length)];
  for (const item of items) {
    if (item.kind === "function_call") {
      ...
      encoded.push(varuint(1), u32(3), encodeText(item.content));
    } else {
      encoded.push(varuint(2), encodeText(item.summary));
    }
  }
  return Buffer.concat(encoded);
}
...
function encodeOutput(items, digest) {
  return Buffer.concat([varuint(0), encodeOutputItems(items), Buffer.from(digest)]);
}

function encodeDecodedAction(decoded) {
  if (decoded.kind === "decoded") {
    return Buffer.concat([varuint(0), decoded.actionBytes]);
  }
  return Buffer.concat([varuint(1), u32(argumentDecodeFailures[decoded.failure])]);
}

function encodeRefusal(text) {
  return Buffer.concat([varuint(1), encodeText(text)]);
}

function encodeTransportFailure(kind) {
  return Buffer.concat([varuint(2), u32(transportFailures[kind])]);
}

function encodeProviderFailure(kind, status) {
  return Buffer.concat([varuint(3), u32(providerFailures[kind]), u16(status)]);
}

function encodeUnsupported(kind) {
  return Buffer.concat([varuint(4), u32(unsupportedResponses[kind])]);
}
...
function decodeVector(bytes, cursor, decode) {
  ...
  assert(length <= bytes.byteLength - cursor.value, "vector length exceeds input");
  const result = [];
...
function encodeBytes(value) {
  ...
  return Buffer.concat([varuint(bytes.byteLength), bytes]);
}

// Reuse the pure Boundary 2 application-value codec for lengths and sum ordinals.
// Declared fixed-width integers and enumeration tags retain their little-endian width.
function readVaruint(bytes, cursor) {
  const decoded = decodeNatural(bytes, cursor.value);
  assert(decoded.value <= BigInt(Number.MAX_SAFE_INTEGER), "length exceeds host capacity");
  cursor.value = decoded.offset;
  return Number(decoded.value);
}

function varuint(value) {
  return Buffer.from(encodeNatural(value));
}
